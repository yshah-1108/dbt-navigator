...
The cold-start measurement pass: DAG shape, terminal versus dead nodes and the three-way dead-model test, orchestrator and BI discovery, dbt version and adapter detection, test-coverage concentration, git activity, and which patterns are grandfathered rather than exemplary. Orchestration goes past locating the config to enumerating platform jobs with their *enabled* state and resolving every selector against the DAG, and CI workflows are classified by purpose rather than listed — the "generates a committed artifact" class inverts other skills' advice from *write this* to *do not hand-edit this*. Owns version detection so five other skills can read a resolved version instead of each re-deriving it. Defers naming inference to `dbt-project-conventions` rather than duplicating it. Ends with a do-not-do-on-arrival section, because the most expensive thing an agent does in a new repository is "clean up" a pattern it has not yet understood.

Its one conditional section is `mapping-the-business.md`: the source-system inventory, the entity map with the authoritative dataset per entity, the cross-system join fabric, and per-mart purpose. Almost none of that is derivable, so the pass is mostly structured asking, and its answers change on the timescale of reorganizations rather than commits — run it when `context.domain_notes` is missing or stale, or when a task turns on business meaning. Sampling real values appears here as an instrument for checking the map, not as the goal; no volume of row counts identifies which system is authoritative.
