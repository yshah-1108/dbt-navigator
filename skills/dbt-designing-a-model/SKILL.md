...
**Which source is authoritative?** When two systems both know a fact and disagree, choosing between them is a business decision with an owner. Never resolve it by preferring the one that is easier to query, and never resolve it by averaging. **Check `context.domain_notes` first** — if the project mapped its source systems, this is already answered per entity, and re-asking a settled question is how the answer drifts.
