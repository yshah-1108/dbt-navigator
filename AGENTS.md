...
**Derive it.** If a connected tool can answer the question, answer it that way — silently, without narrating the lookup. Asking a person something the warehouse already knows is a failure, not diligence. Before asking anything, check it against the derivable classes: project structure and history from the repo and git, schema and grain and cardinality from the warehouse, dependencies and consumers from the DAG and BI metadata, freshness and run outcomes from the orchestrator, cost and query behavior from warehouse query logs, ownership from git history and CODEOWNERS, and **documented definitions, intent and incident history from the organization's wiki, ticket tracker and pull requests**.

That last class is the one most often skipped, and skipping it is what makes an agent's question list twice as long as it needs to be. Business meaning is un-derivable as a *decision* — which of two definitions is canonical is a human call — but the *record* of that decision has frequently been written down already, in a Confluence or Notion data dictionary, in the Jira ticket that prompted a model, or in the PR that changed it. Search there for the metric or entity by name before putting it on the list. A document can be years stale, so treat what you find as **documented, unconfirmed** rather than as fact — but "your wiki says revenue is recognized on delivery; still true?" costs someone one word to answer, where "how do you define revenue?" costs them ten minutes.
